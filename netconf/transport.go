...
type Transport interface {
	Send([]byte) error
	Receive() ([]byte, error)
	ReceiveNotification() ([][]byte, error)
	Close() error
...
func (t *transportBasicIO) ReceiveNotification() ([][]byte, error) {
	return t.WaitForBytesNotification([]byte(msgSeperator))
}
...
func (t *transportBasicIO) WaitForFuncNotification(f func([]byte) (int, error)) ([][]byte, error) {
	var out bytes.Buffer
	buf := make([]byte, 4096)
	...
	for {
		...
		if n > 0 {
			...
			if end > -1 && (pos+n)-end == len(msgSeperator) {
				out.Write(buf[0:end])
				return bytes.Split(out.Bytes(), []byte(msgSeperator)), nil
			}
...
func (t *transportBasicIO) WaitForBytesNotification(b []byte) ([][]byte, error) {
	return t.WaitForFuncNotification(func(buf []byte) (int, error) {
		return bytes.LastIndex(buf, b), nil
	})
}
